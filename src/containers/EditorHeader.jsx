...
const EditorHeader = ({ elements, designData, setDesignData }) => {
  ...
  const saveDesign = () => {
    ...
    window.alert(
      "Instance for save design into storage is created, however apology for inconvinience this feature is currently under development."
    );
  };
  ...
  const downloadDesign = () => {
    ...
    downloadLink.setAttribute(
      "download",
      "unifusion_design_" + new Date().toISOString().split("T")[0] + ".json"
    );
      ...
      {previewMode && (
        <div className="preview-container">
          <div className="preview-toolbar">
            ...
            <button
              className={"lol " + `preview-btn ${previewMode ? "active" : ""}`}
              onClick={togglePreview}
            >
              {previewMode ? "Exit Preview" : "Preview"}
            </button>
